...
def requestHour(message):
    ...
    try:
        ...
        if "/stop" in msg:
            ...
            recurso_Usuario = recursos[int(msg)] #EXTRAI DE RECURSOS OS DADOS DA OPÇÃO DO USUARIO
...
def procHour(message):
    ...
    try:
        ...
        if "/stop" in msg:
            ...
            resp = botUtil.isTime(horarios, int(msg), user)
...
def procDate(message):
    data_user = str(message.text).strip().lower()
    user = str(message.from_user.id)
    chat_id = message.chat.id
    ...
    try:
        ...
        if "/stop" in msg:
            ...
            if int(msg) == 1:
                ...
                r = bot.send_message(chat_id,
                                    "Ok, ultima coisa, por questões de segurança o Telegram não me permite  ver seu numero de telelfone, então vou precisar que você mande ele pra mim")
...
def PhoneNumber(message):
    try:
        chat_id = message.chat.id
        user = str(message.from_user.id)
        phone = str(message.text).lower().strip()
